...
MERGE_TEST_DATA_AND_LABELS = True
PREPARE_SMD_ATTACK = False
...
if MERGE_TEST_DATA_AND_LABELS:
    ...
    data_set.to_csv('../smd/SMD_Dataset_Attack.csv', index=True)  # to drop index while writing to csv, set index to False

# No need to standardize across columns ATM
# if PREPARE_SMD_ATTACK:
#     df = pd.read_csv('standardized_smd_attack_df.csv')
...
#     df.to_csv('../SMD_Dataset_Attack.csv', index=True) # to drop index while writing to csv, set index to False
