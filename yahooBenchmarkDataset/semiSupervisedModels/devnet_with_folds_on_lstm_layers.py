...
class DevNet:
    ...
    def run_devnet(self, dataset):
        ...
        # x = scaled_dataset.drop(columns=[ANOMALY_LABEL_COLUMN_NAME, NEW_INDEX_COLUMN]).values
        y = dataset[ANOMALY_LABEL_COLUMN_NAME].values  # y contains anomaly labels
        x = dataset.drop(columns=[ANOMALY_LABEL_COLUMN_NAME, NEW_INDEX_COLUMN]).values
...
if __name__ == '__main__':
    ...
    end_index = len(y) + 1
    history_steps = 150
    x_reshaped, y = create_rnn_univariate_data_with_targetdata(x, y, start_index, end_index, history_steps,
                                                               target_steps)
    ...
    if GRU_or_LSTM == "LSTM":
        ...
        array_for_x_reshaped = np.array(output_for_x_reshaped)
    # Merge array_for_x_reshaped and y, and form a dataset
    df_x = pd.DataFrame(data=array_for_x_reshaped)
    df_y = pd.DataFrame(data=y, columns=[ANOMALY_LABEL_COLUMN_NAME])
    ...
    for train_index, test_index in kf.split(array_for_x_reshaped):
        # print(len(train_index)) # 80% of dataset
        # print(len(test_index)) # 20% of dataset
        test_set = df_result.iloc[test_index[0]:test_index[-1]]
        train_set = df_result.drop(df_result.index[test_index[0]:test_index[-1]])
